...
- **The eval corpus is 100% synthetic and 0% sourced — the macro-F1 numbers are computed against PIL text-on-white test stimuli, not real labels.** The shipped manifest carries 6 PIL-rasterized synthetic fixtures (FIX-01..FIX-04, FIX-06, FIX-07, 200×200 to 480×480 px, plain text on a white canvas using PIL's default bitmap font) plus 14 placeholder `cola-*` rows whose images were never pulled from the TTB Public COLA Registry; `eval/harness.py:run_subset()` silently skips the 14 unfilled rows. The PRD specification (§9.1) and BRD §A-1 / OQ-5 both authorize and assume the CC0-licensed Public COLA Registry as the canonical primary source — that path was researched (T9 §3, T13 §1.0) but never executed under calendar pressure. The shipped state is the canonical anti-pattern called out in PRD §9.1.5 and reframed by D-025: *real-first sourcing is mandatory; pure synthetics are supplements only and must clear the realism bar.* What does ship is honest about itself: the harness, adapter, and scoring are real and complete; provenance fields record `synthetic-*` on every row; the placeholder skip is logged. **Live-stack macro-F1 = 0.19, cost-weighted = 0.94, computed against the 6 FIX fixtures only** — the gap to the ≥0.70 AC is dominated by (a) `gpt-4o-2024-08-06` failing to OCR 200×200 PIL renders (4/6 fixtures route to `ENGINE.EXTRACTION.UNAVAILABLE` and force `needs_review`), and (b) the manifest's `expected_per_rule` using PRD-style `FR-XXX` IDs while the live engine emits YAML-registry IDs (zero overlap, per-rule recall vacuous). To isolate rule-engine quality from OCR robustness, the harness also runs in `--mode replay`: vision is replaced with a synthesizer that maps each fixture's `expected.json` to the rule-side observations validators consume. **Replay macro-F1 = 0.41** on the same 6 fixtures (down from 0.82 pre-fix; the drop is the right direction — the prior 0.82 was inflated by `BRAND.PRESENCE.MISSING` and `ALCOHOL_CONTENT.FORMAT.INVALID` falsely firing on every fixture because validators stringified dict-shaped observations into garbage; that pattern happened to align with the ground-truth `fail`/`needs_review` labels by accident; once the validators became correct, the false-positive rule fires disappeared, replay predictions shifted, and the alignment broke). Replay is a diagnostic isolation tool, not a §4 gate substitute — six layout/verbatim rules are skipped because they need pixel-derived metadata (full disabled list with rationale in `eval/_vision_replay.py`). Cost-weighted = 0.94 means residual errors are false-rejects (asked for review) rather than false-accepts (silently passed). **The credible-measurement unblock is a real-image corpus pass per PRD §9.1.1's mandatory sourcing checklist** — 30–50 hand-curated Registry entries at zero licensing cost, ~4–6 hours of curator time. Full sourcing survey in `docs/research/T13-output.md`; ADR retrospective in `docs/decisions-extended.md` D-023 / D-025; eval corpus self-disclosure in `eval/datasheet.md` §2.
