...
- **The full eval AC gate (macro-F1 ≥ 0.70) is not met on first run.** The harness, adapter, and scoring are all real and complete; the gap is fixture quality and a manifest schema mismatch (the manifest uses `FR-XXX` rule IDs, the live engine uses YAML registry IDs, so per-rule recall computes on an empty intersection). Full write-up in `docs/plans/ttb-label-verification-epoch-8-demo-eval-deploy.md` §10. Cost-weighted score is 0.944, which suggests the failures are FRs (asked for review) rather than FPs (silently passed).
