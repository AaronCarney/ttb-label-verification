...
- **The full eval AC gate (macro-F1 ≥ 0.70) is not met end-to-end.** End-to-end on the live stack: macro-F1 = 0.19, cost-weighted score = 0.94. The harness, adapter, and scoring are real and complete; the gap is two-layered — synthetic 200×200 fixtures aren't reliably OCR-able by `gpt-4o-2024-08-06` (4/6 fixtures route to `ENGINE.EXTRACTION.UNAVAILABLE` and force `needs_review`), and the manifest's `expected_per_rule` uses PRD-style `FR-XXX` rule IDs while the live engine emits YAML-registry IDs (zero overlap, so per-rule recall is vacuous). The post-bold-detection follow-up fixes (audit-key strip on the wire envelope; `fuzzy_brand` and `regex_match` projecting dict-shaped `observed_value` plus reporting `NOT_APPLICABLE` when `expected.value` is missing; degenerate-bbox fallback so `measure_heading_bold` actually runs on real labels) addressed correctness bugs in the rule engine without shifting the live macro-F1 — those bugs weren't load-bearing on the disposition the harness scores against. To isolate rule-engine quality from OCR robustness, the harness also runs in `--mode replay`: vision is replaced with a synthesizer that maps each fixture's `expected.json` to the rule-side observations validators consume. **Replay macro-F1 = 0.41** (down from 0.82 pre-fix). The drop is the right direction: the prior 0.82 was inflated by `BRAND.PRESENCE.MISSING` and `ALCOHOL_CONTENT.FORMAT.INVALID` falsely firing on every fixture (the validators stringified dict-shaped observations into garbage that flunked the format regex and the empty-default fuzzy threshold) — that pattern happened to align with the ground-truth `fail`/`needs_review` labels by accident. Once the validators became correct, those false-positive rule fires disappeared, replay predictions shifted, and the alignment broke. Replay is a diagnostic isolation tool, not a §4 gate substitute — six layout/verbatim rules are skipped because they need pixel-derived metadata (full disabled list with rationale in `eval/_vision_replay.py`). Cost-weighted score on the live stack remains 0.94, meaning the residual errors are false-rejects (asked for review) rather than false-accepts (silently passed). The single biggest credible-measurement gap is fixture corpus size — the manifest covers 6 demo labels plus 14 placeholder COLA entries whose images are not shipped; expanding the corpus is the next move. Full write-up in `docs/plans/ttb-label-verification-epoch-8-demo-eval-deploy.md` §10.
