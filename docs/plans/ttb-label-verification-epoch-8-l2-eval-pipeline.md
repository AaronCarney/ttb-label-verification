...
**Authoring sequence is real-first per D-025 / PRD §9.1.1.** Every iteration of the manifest must follow the five-step ordering — Registry pulls precede synthetic generation, and any gap is recorded explicitly:

1. **Pull from TTB Public COLA Registry first** (T9 §3 / T13 §1.0; CC0). Curate ~30–40 entries across spirits / wine / malt with stratified ABV, brand, and applicant variance. Save each image under `fixtures/_corpus/cola-{14-char-ttbid}/label.{png,jpg}` and record `provenance.source = cola-{ttbid}`. **This step happens before any synthetic build script runs.**
2. **Pull negatives from PACER/CourtListener** (T13 §1.6) for brand and geographic-claim rules — 5–20 entries, `provenance.source = pacer-{case-id}` or `courtlistener-{docket-id}`.
3. **Apply controlled degradation to Registry images** for the borderline slice (mild blur, glare, JPEG compression, perspective shift). Each degraded entry records `provenance.source = synthetic-derived-from-cola-{ttbid}@{degradation-script-sha}` — preserving CC0 lineage and verifiable ground truth. Cap at the ≤30% synthetic share.
4. **Pure-synthetic supplements last** and only when steps 1–3 cannot fill a per-rule positive coverage gap. Every pure synthetic must clear PRD §9.1.4 — label-shaped renders on paper/cream stock with type hierarchy and a frame, never plain text on white. Provenance: `synthetic-{slug}@{build-script-sha}`. The v0.1–v0.6 PIL-default-bitmap-font-on-white fixture style (`scripts/build_synthetic_fixture*.py`) is the canonical anti-pattern (PRD §9.1.5) and **must not be used as a template** for new work.
5. **Document gaps.** If real-image floor (≥70%) is not met, this L2's executor records *which* sources were attempted, *why* they fell short, and *which* follow-up unblocks the gap. Calendar pressure is not a sufficient waiver — Registry curation for a defensible 30–50 entry corpus is 4–6 hours of curator time at zero licensing cost.

> **Note on COLA IDs.** Registry-sourced labels set `provenance.source` to the public COLA TTB ID (e.g. `cola-22148001000123`); file refs point at locally-cached copies under `fixtures/_corpus/<id>/`. The `fixtures/_corpus/` directory is part of the deliverable, not a placeholder — empty rows whose images aren't sourced are a defect, not a deferred follow-up.

> **Anti-pattern to avoid (D-025 / PRD §9.1.5).** The first execution of this L2 shipped 0 sourced images and 14 placeholder `cola-*` rows whose images were never pulled; `eval/harness.py:run_subset()` silently skipped them. The result: macro-F1 in the README §Trade-offs is computed against the 6 FIX fixtures only, the demo Space shows text-on-white as "the label being judged," and the BRD §A-1 / OQ-5 reasoning ("the public COLA Registry is a reasonable test corpus") is undefended. **Future runs of this L2 must satisfy step 1 above before authoring step 4.**
...
**Real-first ordering per D-025 / PRD §9.1.1.** TTB Public COLA Registry images are the primary source (T9 §3, T13 §1.0; CC0-licensed) — pulled per-record via the `viewColaDetails.do?action=publicDisplaySearchBasic&ttbid={14-char-ttbid}` URL pattern and stored locally under `fixtures/_corpus/cola-{ttbid}/`. Negative-class entries for brand/geographic rules are sourced from PACER/CourtListener litigation exhibits (T13 §1.6). Borderline-band entries are produced by controlled degradation of Registry images and carry `provenance.source = synthetic-derived-from-cola-{ttbid}@{script-sha}`. Pure-synthetic supplements (only after real-first sourcing has been attempted) clear PRD §9.1.4 — label-shaped renders with paper/cream stock, type hierarchy, and a frame — never plain text on white. The v0.1–v0.6 PIL-text-on-white fixtures (`scripts/build_synthetic_fixture*.py`) are flagged as the canonical anti-pattern in PRD §9.1.5; they remain in the repo for backwards-compatible test wiring but no future fixture work uses them as a template.
