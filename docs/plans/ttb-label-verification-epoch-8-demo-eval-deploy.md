...
1. **AC-§8.4 macro-F1 ≥ 0.70 — NOT MET END-TO-END (live = 0.190).** Root cause is two-layered: (a) synthetic 200×200 PNG fixtures (`PIL.ImageFont.load_default()` text on flat backgrounds) are not consistently OCR-able by `gpt-4o-2024-08-06`; ~4/6 evaluable fixtures route to `ENGINE.EXTRACTION.UNAVAILABLE` and force `needs_review`. (b) Manifest `expected_per_rule` uses PRD-style `FR-XXX` rule IDs (T2 schema) while the live rule engine emits YAML-registry IDs (`spirits.alcohol.tolerance_band` etc.) — no rule_id overlap, so the per-rule recall assertions in `tests/test_eval_full.py` are vacuously satisfied. **Substrate is complete** (live `_live_evaluator` adapter, fixed `importorskip`, history serialization, `eval/history/2026-05-04T20-41-27.255236+00-00.json` produced). To isolate rule-engine quality from OCR robustness as a diagnostic, the harness also runs `--mode replay` (`eval/_vision_replay.py`): vision is replaced with a synthesizer that maps each fixture's `expected.json` to the rule-side observations validators consume; six layout/verbatim rules that need pixel-derived metadata are marked `disabled` with documented rationale. **Replay macro-F1 = 0.822** (5/6 fixtures correct; FIX-07 borderline-confidence isn't replay-encodable since its signal lives in confidence aggregation downstream of validators; history at `eval/history/2026-05-04T21-31-29.645883+00-00.json`). Replay is **not a §4 gate substitute** — the gate was specified end-to-end. It is a sanity check that the rule-engine + disposition logic work correctly when fed clean inputs, separating "rules quality" from "OCR/fixture-fidelity bottleneck". **Path forward** (post-take-home): rebuild fixtures at higher resolution with cleaner typography, OR add a manifest-rule-id ↔ registry-rule-id mapping layer in the harness. Cost-weighted score = 0.83–0.94 (most errors are FR rather than FP, so the cost-aware metric is healthy).
