...
- `app/rules/models.py` — `RuleSet`, `RuleDefinition`, `MatchPolicy` enum, `ReasonCodeEntry`, `AssetRef`, `DecisionTable` are canonically declared in `app/schemas/rules.py` (E1, per E1 §4 exit-gate item 9); `app/rules/models.py` re-exports them for namespace ergonomics so rule-engine code can `from app.rules.models import RuleSet` without reaching across packages.
